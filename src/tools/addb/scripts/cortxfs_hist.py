...
def gen_perfc_op_hist_graph(fn_tag: str="fsal_read", op_file: str="cortxfs_perfc_graph"):
    ...
    '''
    print ("yvals_time: {0}".format(yvals_time))
    print ("yvals_time_fsal: {0}".format(yvals_time_fsal))
    print ("yvals_time_kvs: {0}".format(yvals_time_kvs))
    print ("yvals_time_cfs: {0}".format(yvals_time_cfs))
    print ("yvals_time_dstore: {0}".format(yvals_time_dstore))
    print ("yvals_time_ds: {0}".format(yvals_time_ds))
    print ("yvals_time_cortx_kvs: {0}".format(yvals_time_cortx_kvs))
    print ("yvals_time_m0: {0}".format(yvals_time_m0))
    print ("yvals_time_init: {0}".format(yvals_time_init))
    print ("yvals_time_m0kvs: {0}".format(yvals_time_m0kvs))
    print ("yvals_time_m0_key_iter: {0}".format(yvals_time_m0_key_iter))
    '''
    ...
    if sm_on not in "NA":
        if filter_op_id != -1:
            rects3 = ax.bar(x + width/2 + 1, yvals_time_kvs, width, label='kvs')
            rects4 = ax.bar(x + width/2 + 2, yvals_time_cfs, width, label='cfs')
            rects5 = ax.bar(x + width/2 + 3, yvals_time_dstore, width, label='dstore')
            rects6 = ax.bar(x + width/2 + 4, yvals_time_ds, width, label='ds')
            rects7 = ax.bar(x + width/2 + 5, yvals_time_cortx_kvs, width, label='cortx-kvs')
            rects8 = ax.bar(x + width/2 + 6, yvals_time_m0, width, label='m0')
            rects9 = ax.bar(x + width/2 + 7, yvals_time_init, width, label='init')
            rects10 = ax.bar(x + width/2 + 8, yvals_time_m0kvs, width, label='m0kvs')
            rects11 = ax.bar(x + width/2 + 9, yvals_time_m0_key_iter, width, label='m0_key_iter')
        else:
            rects3 = ax.bar(x, yvals_time_kvs, width, label='kvs')
            rects4 = ax.bar(x, yvals_time_cfs, width, label='cfs')
            rects5 = ax.bar(x, yvals_time_dstore, width, label='dstore')
            rects6 = ax.bar(x, yvals_time_ds, width, label='ds')
            rects7 = ax.bar(x, yvals_time_cortx_kvs, width, label='cortx-kvs')
            rects8 = ax.bar(x, yvals_time_m0, width, label='m0')
            rects9 = ax.bar(x, yvals_time_init, width, label='init')
            rects10 = ax.bar(x, yvals_time_m0kvs, width, label='m0kvs')
            rects11 = ax.bar(x, yvals_time_m0_key_iter, width, label='m0_key_iter')
    '''
    rects1 = ax.bar(x + width/2, yvals_time, width, label='execution_time')
    rects3 = ax.bar(x + width/(0.20), yvals_time_kvs, width, label='kvs')
    rects4 = ax.bar(x + width/(0.18), yvals_time_cfs, width, label='cfs')
    rects5 = ax.bar(x + width/(0.16), yvals_time_dstore, width, label='dstore')
    rects6 = ax.bar(x + width/(0.14), yvals_time_ds, width, label='ds')
    rects7 = ax.bar(x + width/(0.12), yvals_time_cortx_kvs, width, label='cortx-kvs')
    rects8 = ax.bar(x + width/(0.10), yvals_time_m0, width, label='m0')
    rects9 = ax.bar(x + width/(0.8), yvals_time_init, width, label='init')
    rects10 = ax.bar(x + width/(0.6), yvals_time_m0kvs, width, label='m0kvs')
    rects11 = ax.bar(x + width/(0.4), yvals_time_m0_key_iter, width, label='m0_key_iter')
    '''
    ...
    if sm_on not in "NA":
        if filter_op_id != -1:
            ...
            autolabel(rects8, yvals_time_m0)
            autolabel(rects9, yvals_time_init)
            autolabel(rects10, yvals_time_m0kvs)
            autolabel(rects11, yvals_time_m0_key_iter)
    fig.tight_layout()
